            ...
            switch(uiAction) {
                ...
                default:
                {
                    if (uiAction > ARENA_START_TEAM_LOOKUP) 
                    {
                        ...
                        // no team found
                        if(!result) {
                            player->GetSession()->SendNotification("A equipe de arena não foi encontrada.");
                            player->PlayerTalkClass->SendCloseGossip();
                            return true;
                        }
                        ...
                        if(!members) {
                            player->ADD_GOSSIP_ITEM(GOSSIP_ICON_CHAT, "A equipe não tem membros.", GOSSIP_SENDER_MAIN, parentOption);
                        } else {
                            uint32 memberPos = 1;
