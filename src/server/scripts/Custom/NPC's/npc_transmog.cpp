    ...
    public:
        ...
        bool OnGossipHello(Player* player, Creature* creature) override
        {
            ...
            if (sTransmogrification->EnableSets)
                AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/RAIDFRAME/UI-RAIDFRAME-MAINASSIST:30:30:-18:0|tConfigurar conjuntos", EQUIPMENT_SLOT_END + 4, 0);
#endif
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tRemover todas as transmogrificação", EQUIPMENT_SLOT_END + 2, 0, "Remover todas as transmogrificações de todos os items equipados?", 0, false);
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|tAtualizar", EQUIPMENT_SLOT_END + 1, 0);
            ...
            switch (sender)
            {
                ...
                case EQUIPMENT_SLOT_END + 4: // Presets menu
                {
                    ...
                    if (sTransmogrification->EnableSetInfo)
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Book_11:30:30:-18:0|tComo funcionam os conjuntos", EQUIPMENT_SLOT_END + 10, 0);
                ...
                case EQUIPMENT_SLOT_END + 6: // view preset
                {
                    ...
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Misc_Statue_02:30:30:-18:0|tUsar conjunto", EQUIPMENT_SLOT_END + 5, action, "Usando este conjunto para transmogrify irá ligar itens transmogrified para você e torná-los não reembolsáveis e não negociáveis.\nVocê deseja continuar?\n\n" + it->second.name, 0, false);
                ...
                case EQUIPMENT_SLOT_END + 8: // Save preset
                {
                    ...
                    if (canSave)
                        AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/GuildBankFrame/UI-GuildBankFrame-NewTab:30:30:-18:0|tSalvar conjunto", 0, 0, "Inserir nome do conjunto", cost*sTransmogrification->SetCostModifier + sTransmogrification->SetCopperCost, true);
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|tAtualizar", sender, action);
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tVoltar..", EQUIPMENT_SLOT_END + 4, 0);
                    SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
                } break;
        ...
        void ShowTransmogItems(Player* player, Creature* creature, uint8 slot) // Only checks bags while can use an item from anywhere in inventory
        {
            ...
            if (oldItem)
            {
                ...
                for (uint8 i = INVENTORY_SLOT_ITEM_START; i < INVENTORY_SLOT_ITEM_END; ++i)
                {
                    ...
                    if (sTransmogrification->GetFakeEntry(oldItem) == newItem->GetEntry())
                        continue;
                    ++limit;
                    AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, sTransmogrification->GetItemIcon(newItem->GetEntry(), 30, 30, -18, 0) + sTransmogrification->GetItemLink(newItem, session), slot, newItem->GetGUID().GetCounter(), "Usando este item para transmofica-lo irá vinculá-lo a você e torná-lo não reembolsável e não negociável.\nVocê deseja continuar?\n\n" + sTransmogrification->GetItemIcon(newItem->GetEntry(), 40, 40, -15, -10) + sTransmogrification->GetItemLink(newItem, session) + ss.str(), price, false);
                }
            ...
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/INV_Enchant_Disenchant:30:30:-18:0|tRemover transmogrificação", EQUIPMENT_SLOT_END + 3, slot, "Remover transmogrificação do slot?", 0, false);
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/PaperDollInfoFrame/UI-GearManager-Undo:30:30:-18:0|tAtualizar", EQUIPMENT_SLOT_END, slot);
            AddGossipItemFor(player, GOSSIP_ICON_MONEY_BAG, "|TInterface/ICONS/Ability_Spy:30:30:-18:0|tVoltar..", EQUIPMENT_SLOT_END + 1, 0);
            SendGossipMenuFor(player, DEFAULT_GOSSIP_MESSAGE, creature->GetGUID());
        }
    };
