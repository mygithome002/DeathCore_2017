...
void Player::FitPlayerInTeam(bool action, Battleground* pBattleGround)
{
    ...
    if (pBattleGround && action)
        SendChatMessage("%sVocê está jogando para a %s%s na batalha de %s", MSG_COLOR_WHITE, GetTeam() == ALLIANCE ? MSG_COLOR_DARKBLUE"aliança" : MSG_COLOR_RED"horda", MSG_COLOR_WHITE, pBattleGround->GetName());
}
