	...
	case 0:
		...
		for (SessionMap::const_iterator itr = sessions.begin(); itr != sessions.end(); ++itr)
		{
			if (itr->second->GetSecurity() > IgnoreAccountRank)
				itr->second->SendNotification("|CFFFFFF00[FineWoW Servers - Sistema Anticheat - Antihacker] :: |r |CFFFFFFFF[%s]|r Pode estar usando %s, verifique isso!", player->GetName().c_str(), str.c_str());
		}
		break;
	case 1:
		...
		{
			if (itr->second->GetSecurity() < IgnoreAccountRank)
				itr->second->SendNotification("|CFFFFFF00[FineWoW Servers - Sistema Anticheat - Antihacker] :: |r |CFFFFFFFF[%s]|r Foi parado e mandado para a Sala Oficial do Administrador por usar %s!", player->GetName().c_str(), str.c_str());
		}
		break;
	case 2:
		...
		{
			if (itr->second->GetSecurity() < IgnoreAccountRank)
				itr->second->SendNotification("|CFFFFFF00[FineWoW Servers - Sistema Anticheat - Antihacker] :: |r |CFFFFFFFF[%s]|r Foi desconectado por usar %s!", player->GetName().c_str(), str.c_str());
		}
		break;
	case 3:
		...
		{
			if (itr->second->GetSecurity() < IgnoreAccountRank)
				itr->second->SendNotification("|CFFFFFF00[FineWoW Servers - Sistema Anticheat - Antihacker] :: |r |CFFFFFFFF[%s]|r Se suicidou por usar %s!", player->GetName().c_str(), str.c_str());
		}
		break;
	case 4:
		...
		{
			if (itr->second->GetSecurity() < IgnoreAccountRank)
				itr->second->SendNotification("|CFFFFFF00[FineWoW Servers - Sistema Anticheat - Antihacker] :: |r |CFFFFFFFF[%s]|r Levou Stum e Dormiu por usar %s!", player->GetName().c_str(), str.c_str());
		}
		break;
    }
 }
...
void AnticheatMgr::Report(Player* player, uint8 hackType)
{
	...
	default:
		if (m_Players[key].GetReports() > 1)
			ChatHandler(player->GetSession()).PSendSysMessage("|CFFFF0000[FineWoW Servers - Sistema Anticheat - Antihacker] :: Detectando uso de Cheat - Hacker [ %u / %u ] para ser punido.|r", m_Players[key].GetReports(), MaxReports);
		if (m_Players[key].GetReports() >= MaxReports)
			Punish(player, hackType);
		break;
	}
